...
const columns: ColumnDef<PayInTransaction>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2 text-text-primary">
          ...
          {method === "Transfer" && <ArrowRightLeft size={14} />}
          {method === "Crypto" && <Bitcoin size={14} />}
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          className={cn(
            "rounded-full px-3 py-1 font-bold text-[10px] border-0",
  ...
  },
  {
    ...
    cell: () => (
      <Button variant="ghost" size="icon" className="size-8 text-text-muted hover:bg-surface-2">
    ...
    <div className="flex flex-col rounded-4xl border border-surface-3 bg-white shadow-sm overflow-hidden">
      <section className="w-full">
        <DataTable
          ...
          headerRowClassName="bg-white border-b border-surface-3"
          headClassName="h-14 px-8 py-4 font-bold"
          bodyRowClassName="border-b border-surface-2 last:border-0 transition-colors hover:bg-surface-2"
          bodyCellClassName="px-8 py-6"
        />
      </section>

      <div className="flex items-center justify-between p-8 bg-surface-1">
